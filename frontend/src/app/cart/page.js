...
export default function CartPage() {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen bg-black text-white font-sans p-8 md:p-20 selection:bg-red-600"
    >
      ...
      <main className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-16">
            ...
            {cart.length > 0 ? (
              cart.map((item, index) => (
                <motion.div 
                  ...
                  key={index} 
                  className="bg-gray-900/40 border border-white/5 p-8 rounded-[2rem] flex items-center justify-between group hover:border-red-600/30 transition-all duration-500 shadow-xl"
                >
              ...
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 0.2 }}
                className="text-gray-900 font-black py-32 text-center uppercase tracking-[1.5em] text-2xl select-none italic"
              >
                CART_EMPTY
              </motion.div>
            )}
        ...
        <motion.div 
          ...
          className="bg-gray-950 p-10 rounded-[2.5rem] border border-white/5 h-fit sticky top-32 shadow-2xl relative overflow-hidden"
        >
