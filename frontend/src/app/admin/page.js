...
// Importing Framer Motion for high-end dashboard animations
...
export default function AdminDashboard() {
  ...
  const generatePDF = () => {
    ...
    doc.text("TECH SMART INVENTORY", 14, 20);
    doc.setFontSize(10);
    ...
    doc.save(`TECH_Intelligence_Report_${new Date().getTime()}.pdf`);
  };
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {alerts.length > 0 && (
          <motion.div 
            ...
            className="bg-red-600/10 border border-red-600/30 p-6 rounded-[2.5rem] relative overflow-hidden"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div variants={cardVariants} className="lg:col-span-2 bg-gray-900/50 backdrop-blur-md p-10 rounded-[2.5rem] border border-gray-800 shadow-2xl">
        ...
        {/* --- AI INSIGHTS --- */}
        <motion.div variants={cardVariants} className="bg-red-950/5 border border-red-900/20 p-10 rounded-[2.5rem] flex flex-col justify-between shadow-2xl relative overflow-hidden">
      ...
      <motion.div variants={cardVariants} className="bg-gray-900/30 p-10 rounded-[2.5rem] border border-gray-800 shadow-2xl">
            ...
            {orders.length === 0 ? (
              ...
              orders.map((order) => (
                <motion.div 
                  layout
                  key={order.id} 
                  className="bg-black/40 p-6 rounded-2xl border border-gray-800 flex flex-col md:flex-row justify-between items-center group hover:border-red-600/30 transition-all gap-4"
                >
...
function StatCard({ title, value, color, isAI }) {
  return (
    <motion.div 
      ...
      className={`bg-gray-900/50 p-8 rounded-3xl border border-gray-800 shadow-xl group transition-all relative overflow-hidden`}
    >
