...
export default function InventoryPage() {
  ...
  return (
    <div className="p-8 space-y-12 bg-black min-h-screen text-white font-sans selection:bg-red-600">
      
      {/* --- HEADER SECTION --- */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-gray-900 pb-12">
        <div>
          <h1 className="text-4xl font-black uppercase italic tracking-tighter border-l-8 border-red-600 pl-6 animate-in slide-in-from-left duration-500">
      ...
      <div className="bg-gray-900/50 backdrop-blur-md p-10 rounded-[2.5rem] border border-gray-800 shadow-2xl relative overflow-hidden group transition-all hover:border-red-900/30">
        ...
        <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="flex items-center space-x-4 bg-black border border-gray-800 p-2 rounded-xl">
             ...
             {previewUrl && <img src={previewUrl} alt="Preview" className="w-10 h-10 object-cover rounded-lg border border-gray-700" />}
          </div>
          <button 
            type="submit" 
      ...
      <div className="bg-gray-900 rounded-[2.5rem] border border-gray-800 overflow-hidden shadow-2xl overflow-x-auto">
          ...
          <tbody className="divide-y divide-gray-800/50">
            {filteredProducts.map(p => (
              <tr key={p.id} className="hover:bg-white/[0.03] transition-all group border-b border-transparent">
                <td className="p-8">
                  {/* CORRECTED: Removed bg-black and border-gray-800 to eliminate the 'Back Frame' issue */}
      ...
      {editingProduct && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-gray-900 border-2 border-gray-800 w-full max-w-2xl p-12 rounded-[2.5rem] shadow-3xl relative overflow-hidden selection:bg-red-600">
            <div className="absolute top-0 right-0 w-40 h-40 bg-red-600/5 blur-3xl pointer-events-none"></div>
            <h2 className="text-3xl font-black mb-10 uppercase italic tracking-tighter animate-in slide-in-from-top-4 duration-500">Edit <span className="text-red-600">Product Info</span></h2>
